...
!----------------------------------------------------------------------
      ...
      do i = 1, 4     !run cycle
        do j = 1, 4   !runtime cycle
          ...
          call grib_open_file(infile, pathaddname, 'r', status)
          ...
          call grib_count_in_file(infile, numOfMessages, status)
          if ( status /= GRIB_SUCCESS ) then
          ...
          do k = 1, min(numOfMessages, 178)
            call grib_new_from_file(infile, gribid(k), status)
            if ( status /= GRIB_SUCCESS ) then
            ...
            ! get data values
            call grib_get(gribid(k), 'values', values, status)
            if ( status /= GRIB_SUCCESS ) then
            ...
            call grib_release(gribid(k), status)
            if ( status /= GRIB_SUCCESS ) then
          ...
          call grib_close_file(infile, status)
          if ( status /= GRIB_SUCCESS ) then
      ...
      do i = 1, 4    !run cycle
        do j = 1, 4   !runtime cycle
          ...
          do k = 1, numOfMessages
            ...
            call grib_get(gribid(k), 'shortName', shortName, status)
            if ( status /= GRIB_SUCCESS ) then
            ...
            ! get data values
            call grib_get(gribid(k), 'values', values)
            if ( status /= GRIB_SUCCESS ) then
              write( 6, '(2a, i3, 2a)' ) subname, 'WARNING: can NOT get the ', k, 'th message of GRIB file ', trim(pathaddname)
              ierr = 1
              return
